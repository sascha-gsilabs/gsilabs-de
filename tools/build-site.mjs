...
import { covers } from './fonts.config.mjs'
...
/* The shipped fonts are cut down to the characters this site writes in, which
   makes them a third of their supplied size and means a character outside that
   set has no glyph. The browser does not complain, it quietly substitutes a
   system font for that one letter, which is the kind of thing nobody notices
   until it is in print. So the build looks. See tools/fonts.config.mjs for
   where the line is drawn and how to move it. */
...
for (const file of written) {
  ...
  for (const char of text) {
    if (!char.trim() || covers(char)) continue
    if (!strays.has(char)) strays.set(char, new Set())
    strays.get(char).add(file.replace(/\\/g, '/'))
  }
}

if (strays.size) {
  console.log(`\n${strays.size} character(s) the shipped fonts cannot draw:`)
  for (const [char, files] of strays) {
    const where = [...files].slice(0, 3).join(', ') + (files.size > 3 ? ` and ${files.size - 3} more` : '')
    console.log(`  ${char}  U+${char.codePointAt(0).toString(16).toUpperCase().padStart(4, '0')}  ${where}`)
  }
  console.log('  Add its range in tools/fonts.config.mjs and rerun npm run build:assets.')
}
